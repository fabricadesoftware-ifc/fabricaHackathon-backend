...
class TeamFilter(FilterSet):
    student_id = CharFilter(field_name="students__id", lookup_expr="exact")
    student_name = CharFilter(
        field_name="students__user__name", lookup_expr="icontains"
    )
    student_email = CharFilter(
        field_name="students__user__email", lookup_expr="icontains"
    )
