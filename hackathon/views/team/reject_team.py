...
from hackathon.tasks import send_team_rejection_email_to_students
...
@api_view(["GET"])
def reject_team(request, verification_token) -> Response:
    try:
    ...
    if edition.registration_deadline > timezone.now():
        ...
        send_team_rejection_email_to_students.delay(team.id)
        return Response(
            {"message": "Trabalho recusado."},
            status=status.HTTP_200_OK,
        )

    return Response(
        {"error": "Data limite para recusar o time expirou."},
    )
